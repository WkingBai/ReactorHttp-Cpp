...
int EpollDispatcher::epollCtl(int op)
{
	...
	//channel中events是自己定义的， epoll中events是Linux定义的，需要用判断保证一致性
	int events = 0;
...
int EpollDispatcher::dispatch(int timeout)
{
	...
	for (int i = 0; i < count; ++i) {
		...
		if (events & EPOLLERR || events & EPOLLHUP) {  //EPOLLERR : 对端断开连接   EPOLLHUP : 对端断开连接了，服务器依然在发数据
			//对方断开连接，删除fd
			//remove();
			continue;
		}
